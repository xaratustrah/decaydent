...
def get_item(filename, item_no):
    with h5py.File(filename, 'r') as hdf:
        ...
        if item_no >= n_items_total:
            raise ValueError(
                f'File has only {n_items_total} items in it, possible indexes are between 0 and {n_items_total-1}.')

        bboxes_as_ndarray = np.array(hdf[f'bboxes_as_list/{item_no}'])
        decay_coords_as_array = np.array(
            hdf[f'decay_coords_as_list/{item_no}'])
...
def main():
    ...
    try:
        image, bboxes, instances, bboxes_as_ndarray, decay_coords_as_array = get_item(
            infilename, args.number)
    ...
    plot_pattern(
        f'{args.infilename[0]}_{args.number:03d}', image, bboxes, instances)
